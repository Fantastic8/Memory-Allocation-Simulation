...
public class WorstFit extends AlgorithmSelector<LinkedList<DataItem>> {

	public WorstFit(AnalyzePanel AP,AlgorithmContainer AC)
	{
		this.AP=AP;
		this.AC=AC;
	...
	public synchronized int[] createWork(int size, int processingtime) {
		...
		//search in avaliable table
		if(AvailableTable.size()<=0)
		{
			return new int[]{-1,size};
		}
		DataItem dt=AvailableTable.get(0);//biggest
		if(dt.size>size)
		{
			//split data item
			...
			InsertIntoAvailableTableOrderBySizeB(dt);
	...
	public synchronized void releaseWork(DataItem dt) {
		...
		for(int i=0;i<AvailableTable.size();i++)
		{
			if(AvailableTable.get(i).address+AvailableTable.get(i).size==dt.address)//front emerge
			{
				...
				InsertIntoAvailableTableOrderBySizeB(dt);//insert
	...
	public void InsertIntoAvailableTableOrderBySizeB(DataItem dt)
	{
		for(int i=0;i<AvailableTable.size();i++)
		{
